...
결과: (1) IMU 융합은 복원 충실도(PSNR +0.37~+0.68 dB)를 세 블러 수준 모두 일관되게 \
개선했고, 이 이득은 용량 대조·셔플 대조 양쪽에서 운동 정보의 기여로 귀속된다. \
(2) 인식 손실×IMU의 2×2 절제에서 유의한 상호작용(−5.9%p)이 관찰됐지만, 매개변수를 맞춘 \
대조군에서는 인식 손실이 영상 전용 모델에도 유의하게 도움이 되어(−5.7%p) 용량 통제 후 \
상호작용은 사라졌다 — 인식 손실의 효과는 센서가 아니라 모델 용량에 의존한다. \
(3) 실촬영 32장에서는 모든 복원 구성이 입력을 그대로 읽는 것보다 나빠(+27.7~+30.8%p), \
합성→실측 도메인 갭이 현재의 병목임을 확인했다. 대조군 없는 절제 실험이 상호작용을 \
센서 특이 효과로 오독하게 만들 수 있다는 방법론적 교훈이 본 연구의 핵심 기여다.`,
  ...
  method: {
    ...
    caption:
      ...
      "게이트 교차 어텐션으로 융합한다. 학습 시 고정된 줄 단위 CTC 인식기의 인식 " +
      "손실이 복원 네트워크로 역전파된다(평가는 별도 계열 EasyOCR로 분리).",
    paragraphs: [
  ...
  results: {
    heading: "결과",
    intro:
      "합성 테스트(레벨당 50장, EasyOCR 평가)의 전 구성 비교와, 그 차이가 어디서 오는지를 가른 " +
      "대조군 분해. 아래 슬라이더는 흐린 입력과 복원 결과를 드래그로 비교하며, 버튼으로 " +
      "IMU 없는 복원 ↔ 제안(IMU) 모델을 전환할 수 있다. 합성 성공 사례와 함께, 직접 수집한 " +
  ...
  acknowledgements:
    "정직성 고지: 이 페이지의 모든 수치는 실제 수행된 실험(고정 시드, 텍스트 클러스터 " +
    "부트스트랩 95% CI)에서 가져왔으며 합성 데이터 중심의 탐색적 결과다. 정량 결과는 단일 학습 " +
    "시드·단일 평가 인식기 조건이고, 실촬영에서는 도메인 갭으로 복원 이득이 전이되지 않았다. " +
    "성공 사례와 실패 사례를 함께 전시한다.",
};
